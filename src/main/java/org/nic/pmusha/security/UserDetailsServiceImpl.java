...
import org.nic.pmusha.uservo.UserRolesVO;
...
//		}
		...
		UserMaster fetchUserByUsername = userdao.fetchUserByUsername(username);
		if(fetchUserByUsername!=null) {
			...
			// The "User" class is provided by Spring and represents a model class for user to be returned by UserDetailsService
			// And used by auth manager to verify and check user authentication.
			return new User(fetchUserByUsername.getEmailId(), fetchUserByUsername.getBcryptPassword(), grantedAuthorities);
		}
